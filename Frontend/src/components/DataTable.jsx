...
const buildHiddenSet = (initialCols, savedVisibleKeys) => {
  if (!savedVisibleKeys || !Array.isArray(savedVisibleKeys) || savedVisibleKeys.length === 0)
    return new Set();
  const savedSet = new Set(savedVisibleKeys);
  return new Set(initialCols.filter(c => !savedSet.has(c.key) && c.key !== 'actions').map(c => c.key));
};
...
export default function DataTable({
  ...
  // ── Sync column settings across identical tables ──────────────────────────
  useEffect(() => {
    if (!tableId) return;

    const handleSync = (e) => {
      if (e.detail.tableId === tableId) {
        const savedOrder = e.detail.savedOrder;
        savedOrderRef.current = savedOrder;
        setColumns(buildOrderedColumns(initialColumns, savedOrder));
        setHiddenColumns(buildHiddenSet(initialColumns, savedOrder));
      }
  ...
  const getAbsoluteIndex = (relativeIndex) => {
    if (pageSize === 'all') return relativeIndex;
  ...
  const handleRowDrop = (e, index) => {
    ...
    const fromAbsIdx = getAbsoluteIndex(draggedRowIndex);
    const toAbsIdx = getAbsoluteIndex(index);

    if (onRowReorder) {
      onRowReorder(fromAbsIdx, toAbsIdx, paginatedRows[draggedRowIndex], paginatedRows[index]);
    }
  ...
  const [pageSize, setPageSize] = useState(hidePagination ? 'all' : 8);
  ...
  const filteredRows = useMemo(() => {
    ...
    if (search) {
      filtered = filtered.filter(row => {
        // Collect custom search values from column definitions
        const customValues = initialColumns
          .filter(col => typeof col.getSearchValue === 'function')
          .map(col => col.getSearchValue(row));

        // Collect raw values from the row
        const rawValues = Object.values(row)
          .filter(val => typeof val === 'string' || typeof val === 'number');

        const searchableValues = [...customValues, ...rawValues]
          .join(' ')
          .toLowerCase();
  ...
  const handleDrop = (targetKey) => {
    ...
    if (tableId) {
      ...
      window.dispatchEvent(new CustomEvent('table-preference-updated', { detail: { tableId, savedOrder: newOrder } }));
    }
  };
  ...
  const handleToggleColumn = (key) => {
    ...
    if (tableId) {
      ...
      window.dispatchEvent(new CustomEvent('table-preference-updated', { detail: { tableId, savedOrder: visibleKeys } }));
    }
  };
  ...
  const handleResetColumns = () => {
    ...
    if (tableId) {
      ...
      window.dispatchEvent(new CustomEvent('table-preference-updated', { detail: { tableId, savedOrder: allKeys } }));
    }
  };
        ...
        >
          <table className="min-w-full border-separate border-spacing-0">
            <thead>
              <tr>
                {onRowReorder && (
                  <th className="border-b border-blue-800 bg-blue-900 px-2 py-3 w-8 text-center text-white"></th>
                )}
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={visibleColumns.length + (onRowReorder ? 1 : 0)} className="px-6 py-8 text-center text-slate-500 text-sm">
                    Loading...
                  </td>
                </tr>
              ) : paginatedRows.length > 0 ? (
                paginatedRows.map((row, rowIndex) => {
                  const isDragging = rowIndex === draggedRowIndex;
                  ...
                  return (
                    <tr
                      ...
                      } ${onRowReorder && !row.is_on_hold ? 'hover:shadow-sm' : ''}`}
                    >
                      {onRowReorder && (
                        <td className="border-b border-slate-200 bg-white px-2 py-2 text-center text-slate-400 w-8 select-none">
                          {!row.is_on_hold ? (
                            <i className="fa-solid fa-grip-vertical cursor-grab active:cursor-grabbing text-xs hover:text-slate-600"></i>
                          ) : (
                            <i className="fa-solid fa-lock text-[10px] text-slate-350" title="Held item - locked"></i>
                          )}
                        </td>
                      )}
                      {visibleColumns.map((column) => {
