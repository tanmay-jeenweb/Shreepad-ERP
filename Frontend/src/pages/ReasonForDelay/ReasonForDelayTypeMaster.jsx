...
import {
  ...
  getReasonForDelayTypes,
  updateReasonForDelayType,
  deleteReasonForDelayType,
} from "../../api/reasonForDelayTypeApi";
import DataTable from "../../components/DataTable";
import toast from "react-hot-toast";
import { usePermission } from "../../context/PermissionContext";

export default function ReasonForDelayTypeMaster() {
  ...
  const loadTypes = async () => {
    setLoading(true);
    try {
      const response = await getReasonForDelayTypes();
      setTypes(response.data.data || []);
    } catch (err) {
      console.error("Failed to load reason for delay types", err);
      toast.error("Unable to load reason for delay types. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddType = async (event) => {
    ...
    if (!newName.trim()) {
      toast.error("Enter a valid reason for delay type name.");
      return;
    }

    setSaving(true);
    try {
      await createReasonForDelayType({ reasonTypeName: newName.trim() });
      setNewName("");
      setShowAddModal(false);
      await loadTypes();
      toast.success("Reason for delay type added successfully");
    } catch (err) {
      console.error("Failed to add reason for delay type", err);
      const serverMessage = err?.response?.data?.message;
      toast.error(serverMessage || "Unable to add reason for delay type. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdateType = async (id) => {
    ...
    setSaving(true);
    try {
      await updateReasonForDelayType(id, { reasonTypeName: editingName.trim() });
      toast.success("Reason for delay type updated successfully");
      setEditingId(null);
      setEditingName("");
      await loadTypes();
    } catch (err) {
      console.error("Failed to update reason for delay type", err);
      toast.error(err?.response?.data?.message || "Unable to update reason for delay type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteType = async (id) => {
    if (!window.confirm("Are you sure you want to delete this reason for delay type?")) {
      return;
    }
    ...
    try {
      ...
      toast.success("Reason for delay type deleted successfully");
      await loadTypes();
    } catch (err) {
      console.error("Failed to delete reason for delay type", err);
      toast.error(err?.response?.data?.message || "Unable to delete reason for delay type.");
    } finally {
      setSaving(false);
    }
  };
      ...
      { key: "id", label: "ID", minWidth: "60px" },
      {
        ...
        render: (row) =>
          editingId === row.id ? (
            ...
            <span className="font-semibold text-blue-900">{row.reason_type_name}</span>
          ),
      },
    ];

    const canUpdate = hasPermission("reason_for_delay_type", "update");
    const canDelete = hasPermission("reason_for_delay_type", "delete");
          ...
          <div className="flex items-center justify-start gap-2">
            {editingId === row.id ? (
              ...
              <>
                {canUpdate && (
                  <button
                    onClick={() => handleStartEdit(row.id, row.reason_type_name)}
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 font-sans text-slate-900">
      ...
      <main className="flex-1 flex flex-col w-full mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <DataTable
          tableId="reason_for_delay_type_master"
          title="Reason For Delay Type Master"
          data={types}
          columns={columns}
          loading={loading}
          searchPlaceholder="Search reason for delay types..."
          actionButton={
            hasPermission("reason_for_delay_type", "write") ? (
              <button
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl animate-in zoom-in duration-200">
              <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-xl font-semibold text-slate-900">Add Reason For Delay Type</h2>
                  <p className="text-sm text-slate-500">Create a new reason for delay type entry.</p>
                </div>
                <button
                  type="button"
              ...
              <form className="space-y-4" onSubmit={handleAddType}>
                <div>
                  ...
                  <input
                    ...
                    placeholder="Enter reason type name"
                    autoFocus
                  />
                </div>
