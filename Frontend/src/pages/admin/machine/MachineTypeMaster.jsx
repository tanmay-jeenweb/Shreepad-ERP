...
import { createMachineType, getMachineTypes, updateMachineType, deleteMachineType } from "../../../api/machineTypeApi";
...
export default function MachineTypeMaster() {
  ...
  const loadMachineTypes = async () => {
    ...
    try {
      ...
      setError("Unable to load machine types. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddType = async (event) => {
    ...
    if (!newTypeName.trim()) {
      setError("Enter a valid machine type name.");
      return;
    }
    ...
    try {
      await createMachineType({ machineTypeName: newTypeName.trim() });
      setMessage(`Machine type '${newTypeName.trim()}' added successfully.`);
      setNewTypeName("");
      setShowAddModal(false);
      await loadMachineTypes();
    } catch (err) {
      console.error("Failed to add machine type", err);
      const serverMessage = err?.response?.data?.message;
      setError(serverMessage || "Unable to add machine type. Please try again.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleUpdateType = async (id) => {
    ...
    try {
      await updateMachineType(id, { machineTypeName: editingName.trim() });
      ...
      setError(err?.response?.data?.message || "Unable to update machine type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDeleteType = async (id) => {
    ...
    try {
      ...
      console.error("Failed to delete machine type", err);
      setError(err?.response?.data?.message || "Unable to delete machine type.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const columns = useMemo(() => {
    const cols = [
      { key: 'id', label: 'ID', minWidth: '60px' },
      {
        ...
        render: (row) => editingId === row.id ? (
          ...
          <span className="font-semibold text-blue-900">{row.machine_type_name}</span>
        )
      },
      {
        key: 'created_at',
    ...
    const canUpdate = hasPermission("machine_type", "update");
    const canDelete = hasPermission("machine_type", "delete");
          ...
          <div className="flex items-center justify-start gap-2">
            {editingId === row.id ? (
              ...
              <>
                {canUpdate && (
                  <button
                    onClick={() => handleStartEdit(row.id, row.machine_type_name)}
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 font-sans text-slate-900">
      ...
      <main className="flex-1 flex flex-col w-full mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        <DataTable
          tableId="machine_type_master"
          title="Machine Type Master"
          data={machineTypes}
          columns={columns}
          loading={loading}
          searchPlaceholder="Search machine types..."
          actionButton={hasPermission("machine_type", "write") ? (
            <button
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4">
            <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
              <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-xl font-semibold text-slate-900">Add Machine Type</h2>
                  <p className="text-sm text-slate-500">Create a new machine type and store it in the backend.</p>
                </div>
                <button
                  type="button"
              ...
              <form className="space-y-4" onSubmit={handleAddType}>
                <div>
                  <label className="mb-2 block text-sm font-semibold text-slate-700">Machine Type Name</label>
                  <input
                    type="text"
                    value={newTypeName}
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  >
                    {saving ? "Saving..." : "Add Machine Type"}
                  </button>
                </div>
              </form>
            </div>
