...
import { getAllSOsForApproval, approveSO, rejectSO, getApprovalLogs } from "../../../api/soApprovalApi";
...
export default function SOApproval() {
    ...
    const [viewingSoId, setViewingSoId] = useState(null);

    const fetchOrders = async () => {
        setLoading(true);
        try {
            const res = await getAllSOsForApproval();
            setOrders(res.data?.data || []);
        } catch {
            toast.error("Failed to load Sales Order Approvals");
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleApprove = async (id) => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || "Failed to approve Sales Order");
        } finally {
    ...
    const handleRejectSubmit = async (e) => {
        ...
        setActionLoading(true);
        try {
            await rejectSO(rejectingId, rejectReason.trim());
            ...
            },
            {
                ...
                render: (row) => (
                    <div className="flex items-center gap-2">
                        ...
                        {row.status === "pending" && hasPermission("so_approval", "update") && (
                            <>
                                <button
                        ...
                        <button
                            ...
                            className="p-1.5 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors ml-auto cursor-pointer"
                        >
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
            <Navbar title="Sales Order Approvals" />
            ...
            <main className="flex-1 p-4 sm:p-6 lg:p-8 mx-auto w-full">
                    ...
                    {tabs.map(tab => {
                        ...
                        return (
                            <button
                                ...
                                className={`
                                    flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all border cursor-pointer
                                    ${isActive 
                                        ? activeClass 
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                    <DataTable
                        ...
                        columns={columns}
                        loading={loading}
                        searchPlaceholder="Search by Sales Order ID or customer..."
                    />
                </div>
            </main>
                        ...
                        <form onSubmit={handleRejectSubmit}>
                            ...
                            <div className="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
                                <button
                                    type="button"
                                    onClick={() => setRejectModalOpen(false)}
                                    className="px-4 py-2 text-sm font-semibold text-slate-600 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer"
                                >
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    disabled={actionLoading}
                                    className="px-4 py-2 text-sm font-semibold text-white bg-rose-600 border border-transparent rounded-lg hover:bg-rose-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 cursor-pointer"
                                >
                                    {actionLoading ? (
                                        <>
            ...
            {/* View Modal */}
            {viewingSoId && (
                <SalesOrderViewModal soId={viewingSoId} onClose={() => setViewingSoId(null)} />
            )}
        </div>
    );
}
