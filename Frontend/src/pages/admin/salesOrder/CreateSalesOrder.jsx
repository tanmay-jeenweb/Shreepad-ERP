...
export default function CreateSalesOrder() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Failed to load data for sales order", err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(`Sales Order '${form.salesOrderId}' created successfully.`);
      setTimeout(() => navigate("/sales/sales-orders"), 900);
    } catch (err) {
      console.error("Failed to create sales order", err);
      setError(err?.response?.data?.message || "Unable to create sales order. Please try again.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <main className="mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 mt-1 text-sm">Add a new sales order with multiple items.</p>
          </div>
          <button
