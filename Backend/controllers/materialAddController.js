...
const updateMaterialAddHandler = async (req, res) => {
    try {
        ...
        if (ma.status !== "received" && ma.status !== "Pending") {
            return res.status(400).json({ success: false, message: "Cannot edit Material Add after QC operation." });
        }
...
const deleteMaterialAddHandler = async (req, res) => {
    try {
        ...
        if (!ma) return res.status(404).json({ success: false, message: "Material Add not found" });
        if (ma.status !== "received" && ma.status !== "Pending") {
            return res.status(400).json({ success: false, message: "Cannot delete Material Add after QC operation." });
        }
